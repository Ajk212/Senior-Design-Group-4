...
// void my_ack_callback(const char *gesture, bool success)
// {
//     ESP_LOGI("APP", "Gesture %s %s", gesture, success ? "acknowledged" : "failed");
// }
...
void app_main(void)
{
    ...
    // // Set ACK callback
    // ble_service_set_ack_callback(my_ack_callback);
    ...
    while (1)
    {
        ...
        for (int i = 0; i < sizeof(sensor_configs) / sizeof(sensor_configs[0]); i++)
        {
            ...
            // {
            //     ble_service_send_gesture_with_ack("String", 1000);
            // }
        }
